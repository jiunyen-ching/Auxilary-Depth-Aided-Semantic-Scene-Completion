...
from keras.layers import Input, concatenate, Conv3D, Conv3DTranspose, Add, MaxPooling3D, Activation, BatchNormalization, UpSampling3D
from keras.initializers import RandomNormal

def build_mbllen(input_shape=(240,144,240,1)):

    def EM(input, kernal_size, channel):
        conv_1 = Conv3D(channel, (3, 3), activation='relu', padding='same', data_format='channels_last')(input)
        conv_2 = Conv3D(channel, (kernal_size, kernal_size), activation='relu', padding='valid', data_format='channels_last')(conv_1)
        conv_3 = Conv3D(channel*2, (kernal_size, kernal_size), activation='relu', padding='valid', data_format='channels_last')(conv_2)
        conv_4 = Conv3D(channel*4, (kernal_size, kernal_size), activation='relu', padding='valid', data_format='channels_last')(conv_3)
        conv_5 = Conv3DTranspose(channel*2, (kernal_size, kernal_size), activation='relu', padding='valid', data_format='channels_last')(conv_4)
        conv_6 = Conv3DTranspose(channel, (kernal_size, kernal_size), activation='relu', padding='valid', data_format='channels_last')(conv_5)
        res = Conv3DTranspose(3, (kernal_size, kernal_size), activation='relu', padding='valid', data_format='channels_last')(conv_6)
        return res
    ...
    FEM = Conv3D(32, (3, 3), activation='relu', padding='same', data_format='channels_last')(down2_pool)
    EM_com = EM(FEM, 5, 8)

    for j in range(3):
        for i in range(0, 3):
            FEM = Conv3D(32, (3, 3), activation='relu', padding='same', data_format='channels_last')(FEM)
            EM1 = EM(FEM, 5, 8)
            EM_com = Concatenate(axis=3)([EM_com, EM1])

    outputs = Conv3D(3, (1, 1), activation='relu', padding='same', data_format='channels_last')(EM_com)
