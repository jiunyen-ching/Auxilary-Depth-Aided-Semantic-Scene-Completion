...
def processDepthImage(z, missingMask, C):
    ...
    # restore x-y-z position (put back as 3-channel array)
    pc = np.zeros([z.shape[0], z.shape[1], 3])
    pc[:,:,0] = X
    pc[:,:,1] = Y
    pc[:,:,2] = Z

    N1, b1 = computeNormalsSquareSupport(z/100, missingMask, normalParam_patchSize[0], 1, C, np.ones(z.shape))
    N2, b2 = computeNormalsSquareSupport(z/100, missingMask, normalParam_patchSize[1], 1, C, np.ones(z.shape))
    N = N1
...
def getPointCloudFromZ(Z, C, s=1):
    ...
    cc_rgb = C[0:2,2] * s       # the first two lines of column-3, x0 and the y0
...
Input:
    ...
    superpixels:  superpixel map to define boundaries that should
                    not be straddled
'''
def computeNormalsSquareSupport(depthImage, missingMask, R, sc, cameraMatrix, superpixels):
    depthImage = depthImage * 100 # convert to centimetres
