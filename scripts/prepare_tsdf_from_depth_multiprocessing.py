...
params = hf._init_params(target_tsdf=240, target_label=60)

# root_path = "/media/mmu/af326b7d-b9b7-4f2e-8630-ed1069f9f08d/depthbin_eval/depthbin/NYUCAD/NYUCADtest"
root_path = "/media/mmu/af326b7d-b9b7-4f2e-8630-ed1069f9f08d/depthbin_eval/depthbin/NYUCAD/NYUCADtrain"
# path = os.path.join(path, 'preprocessed_ching_2')
path = os.path.join(root_path, 'preprocessed_ching_3')
# save_paths = ['tsdf_240', 'f-tsdf_240', 'weights_60', 'label_60']
save_paths = ['tsdf_240']
...
file_idxs = [int(x[3:7]) for x in os.listdir(os.path.join(root_path,'bin'))]
...
def multiprocess(file_idx):
    ...
    bin_file = os.path.join(root_path,'bin','NYU{:04d}_0000.bin'.format(file_idx))
    depth_file = os.path.join(root_path,'depth','NYU{:04d}_0000_depth.png'.format(file_idx))
    _3d_semantic_file = os.path.join(root_path,'3D_semantic_seg','NYU{:04d}_0000.npz'.format(file_idx))
    ...
    assert object_count > 0, "Skip file if object count is zero"

    # Compute TSDF:
    # |-- 1. Pixel-to-Voxel
    depth_data = hf._read_bitshift(depth_file, return_as_float=True)
    mapping = hf._2Dto3D(depth_data, origin, cam_pose, params, return_as='1D', mapping_as='voxel') # pixel-to-voxel mapping
    _map = np.where(mapping != -1)

    vox_binary = np.zeros(params["tsdf_vox_num"], dtype=np.uint8) # 0: empty, 1: surface
    vox_binary[mapping[_map]] = 1

    # |-- 2. Squared Distance Transform (Array version)
    default_value = 255
    vox_tsdf = np.ones(params["tsdf_vox_num"]) * default_value
    ...
    mask = point_cam_z > 0
    vox_idxs, point_cam_x, point_cam_y, point_cam_z = vox_idxs[mask],\
                                                        point_cam_x[mask],\
                                                        point_cam_y[mask],\
                                                        point_cam_z[mask]
    ...
    # keep only pixels within image frame
    mask = (pixel_x >= 0) * (pixel_x < 640) * (pixel_y >= 0) * (pixel_y < 480)
    vox_idxs, point_cam_z, pixel_x, pixel_y = vox_idxs[mask],\
                                                point_cam_z[mask],\
                                                pixel_x[mask],\
                                                pixel_y[mask]
    ...
    vox_idxs, point_cam_z, point_depth = vox_idxs[mask],\
                                            point_cam_z[mask],\
                                            point_depth[mask]
    ...
    if params["tsdf_vox_size"].tolist() != [240,144,240]:
        print("Skipping downsampling vox_label due to size mismatch between TSDF and vox_label")
    ...
    # vox_tsdf_flip = sign * np.maximum(0.001, 1-abs(vox_tsdf))

    # vox_tsdf = vox_tsdf.reshape(240,144,240)
    # file = "/home/mmu/Desktop/NYU{:04d}_0000.ply".format(file_idx)
    # vox_tsdf, ply_file, vox_coords, vox_count = get_scene_properties(file, 'same', 'tsdf', vox_tsdf, vox_res='hr', lite=True)
    # scene_writer(vox_tsdf, ply_file, vox_coords, vox_count, color_tsdf, 'tsdf', True)

    # vox_tsdf_flip = vox_tsdf_flip.reshape(240,144,240)
    # file = "/home/mmu/Desktop/NYU{:04d}_0000.ply".format(file_idx)
    # vox_tsdf_flip, ply_file, vox_coords, vox_count = get_scene_properties(file, 'same', 'tsdf', vox_tsdf, vox_res='hr', lite=True)
    # scene_writer(vox_tsdf_flip, ply_file, vox_coords, vox_count, color_tsdf, 'tsdf', True)

    # file = "/home/mmu/Desktop/NYU{:04d}_0000.ply".format(file_idx)
    # ply_file, vox_coords, vox_count = get_scene_properties(file, 'same', 'weights', out_vox_weights.reshape((60,36,60)), vox_res='lr', lite=True)
    # scene_writer(out_vox_weights.reshape((60,36,60)), ply_file, vox_coords, vox_count, color_weights, 'weights', True)
    #
    # file = "/home/mmu/Desktop/NYU{:04d}_0000.ply".format(file_idx)
    # ply_file, vox_coords, vox_count = get_scene_properties(file, 'same', 'semantic', out_vox_label.reshape((60,36,60)), vox_res='lr', lite=True)
    # scene_writer(out_vox_label.reshape((60,36,60)), ply_file, vox_coords, vox_count, color_suncg, 'semantic', True)

    # Save TSDF
    processed_file = os.path.join(path, 'tsdf_240', 'NYU{:04d}_0000.npz'.format(file_idx))
    np.savez_compressed(processed_file,
                        tsdf=vox_tsdf.reshape(240,144,240,1))

    # processed_file = os.path.join(path, 'f-tsdf_240', 'NYU{:04d}_0000.npz'.format(file_idx))
    # np.savez_compressed(processed_file,
    #                     tsdf=vox_tsdf_flip.reshape(240,144,240,1))

    # if params["tsdf_vox_size"].tolist() == [240,144,240]:
    #     processed_file = os.path.join(path, 'weights_60', 'NYU{:04d}_0000.npz'.format(file_idx))
    #     np.savez_compressed(processed_file,
    #                         weights=out_vox_weights.reshape(60,36,60,1))
    #
    #     processed_file = os.path.join(path, 'label_60', 'NYU{:04d}_0000.npz'.format(file_idx))
    #     np.savez_compressed(processed_file,
    #                         label=out_vox_label.reshape(60,36,60))
